...
void patchs_lv2(){
    ...
    if(uval == 0x80000000003487D0ULL) cfw = 0x440C; else cfw= 0x450C; 
...
volatile int usb_enable = 2;

char usb_pdevice[]= "/dev_usb000";

char usb_path[]= "/dev_usb000/flags/nosleep";		//ファイルパス フラグ
            ...
            while(1) {
                ...
                if(ret == 0) {
                    ...
                    if(n>=9) {
                        ...
                        ret= sysLv2FsStat(usb_path, &stat); // only write in devices with "nosleep" file	usbパスにフラグファイルが存在する場合0を返す
                        ...
                        if(ret == 0) 
                            ret = sysLv2FsOpen(usb_path, SYS_O_WRONLY | SYS_O_CREAT | SYS_O_TRUNC, &fd, 0777, NULL, 0);	//フラグがある場合 ファイル作成
                    }
                    ...
                    if(ret == 0) {
                        ...
                        sysLv2FsWrite(fd, &usb_data, 16ULL, &writed);		//usb_dataが内容
                        sysLv2FsClose(fd);
                    }
...
sys_ppu_thread_t usbwakeup_id;

sys_ppu_thread_t main_id;
...
s32 main(s32 argc, const char* argv[]) 
{
    ...
    sysThreadCreate(&temp_id, thread_temp, NULL, 3069, 0x1000, THREAD_JOINABLE, "FanCtrl_Thread");
